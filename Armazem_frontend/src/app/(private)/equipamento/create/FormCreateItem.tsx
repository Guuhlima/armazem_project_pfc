...
const FormCreateItem = () => {
  ...
  const onSubmit = async (data: ItemFormData) => {
    ...
    if (data.vincularEstoque && quantidade < minimo) {
      await MySwal.fire({
        icon: 'warning',
        title: 'Valores Inválidos',
        text: 'A quantidade inicial não pode ser menor que o mínimo definido para o estoque.',
        background: '#0b0b0b',
        color: '#e5e7eb',
        customClass: { popup: 'rounded-xl' },
      });
    ...
    const equipmentPayload: EquipamentoPayloadProps = {
      ...
      rastreioTipo: data.rastreioTipo,
    };
    ...
    try {
      ...
      if (!itemId) {
        const backendError =
          resEquip?.data?.error ||
          resEquip?.data?.message ||
          'ID não retornado';
        throw new Error(
          `Falha ao obter ID do equipamento criado. Resposta: ${backendError}`,
        );
      }
      ...
      if (data.vincularEstoque && data.estoqueId && quantidade >= 0 && minimo >= 0) {
        try {
          ...
          await api.post(
            `/stockmovi/cadastro/${data.estoqueId}/adicionar-equipamento`,
            stockPayload,
          );
        } catch (err: any) {
          console.error(
            'Erro DETALHADO ao vincular ao estoque:',
            err.response?.data || err.message,
          );
          await MySwal.fire({
            icon: 'warning',
            title: 'Erro ao Vincular',
            text: `O equipamento "${data.nome}" foi criado (ID: ${itemId}), mas falhou ao vincular ao estoque: ${err.response?.data?.error ||
      ...
      const errorMsg =
        ...
        err.response?.data?.message ||
        err.message ||
        'Erro desconhecido ao cadastrar equipamento.';
      await MySwal.fire({
        icon: 'error',
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-zinc-100 dark:bg-black text-zinc-900 dark:text-zinc-100 transition-colors">
      ...
      <div className="relative z-10">
        ...
        <main
          className={`transition-all duration-300 px-4 sm:px-8 py-12 flex justify-center ${sidebarCollapsed ? 'ml-16' : 'ml-60'
            }`}
        >
          <Card className="w-full max-w-2xl bg-card/90 dark:bg-card/85 backdrop-blur-lg border border-border dark:border-blue-800/50 shadow-xl rounded-xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between p-6 bg-gradient-to-r from-muted/50 to-transparent dark:from-blue-950/40 dark:to-transparent border-b border-border dark:border-blue-800/40">
              <div className="space-y-1">
                ...
                <CardTitle className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
                  <PackagePlus className="w-6 h-6 sm:w-7 sm:h-7 text-primary" />{' '}
                  Registrar Item
                </CardTitle>
              </div>
            </CardHeader>
            ...
            <CardContent className="p-6 sm:p-8 space-y-6">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label
                      ...
                      className="text-sm font-medium text-foreground/80"
                    >
                      Quantidade Inicial {vincularEstoque ? '(no Estoque)' : ''}
                    </Label>
                    <Input
                      id="quantidade"
                      type="number"
                  ...
                  <div>
                    <Label
                      ...
                      className="text-sm font-medium text-foreground/80"
                    >
                      Mínimo {vincularEstoque ? '(no Estoque)' : ''}
                    </Label>
                    <Input
                      id="minimo"
                      type="number"
                ...
                {/* Novo campo: Tipo de Rastreamento */}
                <div>
                  <Label
                    htmlFor="rastreioTipo"
                    className="text-sm font-medium text-foreground/80"
                  >
                  ...
                  <select
                    id="rastreioTipo"
                    {...register('rastreioTipo')}
                    className="w-full border border-input rounded-md px-3 py-2 mt-1 text-base bg-background dark:bg-zinc-900 text-foreground focus:border-primary transition-colors ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  >
                    <option value="NONE">
                      Sem rastreio (quantidade total)
                    </option>
                    <option value="LOTE">
                      Por lote e validade (FEFO)
                    </option>
                    <option value="SERIAL">Por número de série</option>
                  </select>
                  {errors.rastreioTipo && (
                    <p className="text-destructive text-xs mt-1">
                      {String(errors.rastreioTipo.message)}
                    </p>
                  )}
                </div>
                            ...
                            >
                              ...
                              {estoques.map((estoque) => (
                                <option key={estoque.id} value={estoque.id}>
                                  {estoque.nome} (ID: {estoque.id})
                                </option>
                              ))}
                            </select>
                            {errors.estoqueId && (
                              <p className="text-destructive text-xs mt-1">
                                {errors.estoqueId.message}
                              </p>
                            )}
                          </div>
                        </div>
