...
async function main() {
  ...
  if (superAdminRoleId) {
    await prisma.usuarioRole.upsert({
      where: { usuarioId_roleId: { usuarioId: admin.id, roleId: superAdminRoleId } },
      update: {},
  ...
  if (equipRoleId) {
    await prisma.usuarioRole.upsert({
      where: { usuarioId_roleId: { usuarioId: equipUser.id, roleId: equipRoleId } },
      update: {},
      create: { usuarioId: equipUser.id, roleId: equipRoleId },
    });
  }

  console.log('✅ Seeds RBAC completas (permissions, roles, grants, usuários, vínculos).');
}
